...
newfont = []
for z in range(0, 96):
	destination = [minimal_font_width[z]]
	for r in range(0, 6):
		for c in range(0, 8):
			fontarray[c][r] = minimal_font_data[z][r] >> (c + 1) & 1
	for r in range(0, 8):
		currentvalue = 0
		for c in range(0, 7):
			currentvalue = currentvalue | (fontarray[r][c] << (7 - c))
		destination.append(currentvalue)
	substring = ", ".join(("0x{0:02X}".format(element))
													for element in destination)
	newfont.append(destination)
	print("["+ substring + "], #" + str(z))
# print(newfont)
# print("Hallo\nNorbert")

print("------------")
oldbold = [0xf8, 0xcc, 0xcc, 0xf8, 0xcc, 0xcc, 0xf8, 0x00]
old=      [0x3c,0x2a,0x2a,0x2e,0x10,0x00]
new=newfont[34][1:8]
for c in oldbold:
